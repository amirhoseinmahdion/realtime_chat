...
function SidebarHeader({ currentUser, onLogout, onOpenProfile }: Readonly<{ currentUser: User; onLogout: () => void; onOpenProfile: () => void }>) {
  const { t } = useLanguage();
  return <header className="flex items-center justify-between gap-3 px-4 py-4 sm:px-5 sm:py-5"><button className="flex min-w-0 flex-1 items-center gap-3 rounded-xl text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-300" onClick={onOpenProfile} title={t("Open profile")} type="button"><UserAvatar name={currentUser.displayName} url={currentUser.avatarUrl} /><div className="min-w-0"><h1 className="truncate text-sm font-semibold tracking-tight">{currentUser.displayName}</h1><p className="truncate text-xs text-slate-500">@{currentUser.username}</p>{currentUser.bio ? <p className="mt-0.5 truncate text-[11px] text-slate-600">{currentUser.bio}</p> : null}</div></button><button aria-label={t("Sign out")} className="grid size-9 place-items-center rounded-xl text-slate-500 transition hover:bg-white/5 hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-300" onClick={onLogout} title={t("Sign out")} type="button"><LogoutIcon /></button></header>;
}

function SearchBox({ isSearching, onChange, query }: Readonly<{ isSearching: boolean; onChange: (value: string) => void; query: string }>) {
  const { t } = useLanguage();
  return <div className="relative"><span className="pointer-events-none absolute inset-y-0 start-3.5 flex items-center text-slate-600"><SearchIcon /></span><input aria-label={t("Search people")} className="h-11 w-full rounded-xl border border-white/8 bg-white/[0.035] px-10 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-teal-300/40 focus:ring-4 focus:ring-teal-300/5" onChange={(event) => onChange(event.target.value)} placeholder={t("Search people")} type="search" value={query} />{isSearching ? <span className="absolute end-3.5 top-3.5 size-4 animate-spin rounded-full border-2 border-slate-600 border-t-teal-300" /> : null}</div>;
}
...
function ConversationList({ conversations, isLoading, onSelect, selectedId, unreadCounts }: Readonly<{ conversations: Conversation[]; isLoading: boolean; onSelect: (id: string) => void; selectedId: string | null; unreadCounts: Record<string, number> }>) {
  ...
  if (!conversations.length) return <SidebarEmpty title={t("No conversations yet")} description={t("Search for someone above to start your first chat.")} />;
  return <div className="p-2" role="list"><p className="px-3 pb-2 pt-2 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-600">{t("Messages")}</p>{conversations.map((conversation) => { const participant = conversation.participant; const label = participant?.displayName ?? conversation.title ?? "Conversation"; const unread = unreadCounts[conversation.id] ?? 0; return <button aria-label={`${label}${unread ? `, ${unread} unread messages` : ""}`} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-start transition ${selectedId === conversation.id ? "bg-teal-300/9" : unread ? "bg-white/[0.025] hover:bg-white/[0.05]" : "hover:bg-white/[0.035]"}`} key={conversation.id} onClick={() => onSelect(conversation.id)} role="listitem" type="button"><span className="relative"><UserAvatar name={label} url={participant?.avatarUrl} />{unread ? <span className="absolute -end-0.5 -top-0.5 size-3 rounded-full border-2 border-[#0b1220] bg-teal-300" /> : null}</span><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className={`truncate text-sm ${unread ? "font-bold text-white" : "font-semibold text-slate-100"}`}>{label}</span><span className="shrink-0 text-[11px] text-slate-600">{formatShortTime(conversation.lastMessage?.createdAt ?? conversation.updatedAt)}</span></span><span className="mt-1 flex items-center gap-2"><span className={`min-w-0 flex-1 truncate text-xs ${unread ? "font-medium text-slate-300" : "text-slate-500"}`}>{conversation.lastMessage?.content ?? `@${participant?.username ?? "new_chat"}`}</span>{unread ? <span className="grid min-w-5 place-items-center rounded-full bg-teal-300 px-1.5 py-0.5 text-[10px] font-bold text-slate-950">{unread > 99 ? "99+" : unread}</span> : null}</span></span></button>; })}</div>;
}
...
function SearchResults({ isLoading, onSelect, pendingUserId, users }: Readonly<{ isLoading: boolean; onSelect: (user: SearchUser) => void; pendingUserId: string | null; users: SearchUser[] }>) {
  ...
  if (!users.length) return <SidebarEmpty title={t("No people found")} description={t("Try another username or display name.")} />;
  return <div className="p-2" role="list"><p className="px-3 pb-2 pt-2 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-600">{t("People")}</p>{users.map((user) => <button className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-start transition hover:bg-white/[0.035] disabled:opacity-60" disabled={pendingUserId === user.id} key={user.id} onClick={() => onSelect(user)} role="listitem" type="button"><UserAvatar name={user.displayName} url={user.avatarUrl} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-100">{user.displayName}</span><span className="mt-0.5 block truncate text-xs text-slate-500">@{user.username}{user.bio ? ` · ${user.bio}` : ""}</span></span><span className="text-xs font-semibold text-teal-300">{pendingUserId === user.id ? "Opening…" : t("Chat")}</span></button>)}</div>;
}
...
function ConversationHeader({ conversation, isOnline, isTyping, onBack }: Readonly<{ conversation: Conversation; isOnline: boolean; isTyping: boolean; onBack: () => void }>) {
  ...
  return <header className="flex h-[73px] shrink-0 items-center gap-3 border-b border-white/8 px-4 sm:px-5"><button aria-label={t("Back to conversations")} className="grid size-9 place-items-center rounded-xl text-slate-400 hover:bg-white/5 lg:hidden" onClick={onBack} type="button"><BackIcon /></button><UserAvatar name={name} size="small" url={conversation.participant?.avatarUrl} /><div className="min-w-0"><h2 className="truncate text-sm font-semibold text-white">{name}</h2><p className={`truncate text-xs ${isTyping || isOnline ? "text-teal-300" : "text-slate-500"}`}>{isTyping ? t("typing…") : isOnline ? t("online") : `@${conversation.participant?.username ?? "conversation"}`}</p></div></header>;
}
...
function MessageHistory({ currentUserId, isLoading, isLoadingOlder, messages, nextCursor, onLoadOlder, onRetry, readMessageId }: Readonly<{ currentUserId: string; isLoading: boolean; isLoadingOlder: boolean; messages: Message[]; nextCursor: string | null; onLoadOlder: () => void; onRetry: (message: Message) => void; readMessageId: string | null }>) {
  ...
  return <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6">{nextCursor ? <div className="mb-5 text-center"><button className="rounded-lg border border-white/8 px-3 py-2 text-xs text-slate-400 transition hover:bg-white/5 hover:text-white disabled:opacity-50" disabled={isLoadingOlder} onClick={onLoadOlder} type="button">{t(isLoadingOlder ? "Loading…" : "Load older messages")}</button></div> : null}{!messages.length ? <div className="grid h-full place-items-center text-center"><div><p className="text-sm font-semibold text-slate-300">{t("A quiet start")}</p><p className="mt-1 text-xs text-slate-600">{t("Send the first message.")}</p></div></div> : <div className="space-y-2" role="log">{messages.map((message) => { const own = message.sender.id === currentUserId; return <div className={`flex ${own ? "justify-end" : "justify-start"}`} key={message.id}><div className={`max-w-[82%] rounded-2xl px-4 py-2.5 sm:max-w-[68%] ${message.delivery === "failed" ? "border border-rose-400/40 bg-rose-400/10 text-rose-100" : own ? "rounded-br-md bg-teal-300 text-slate-950" : "rounded-bl-md border border-white/8 bg-white/[0.045] text-slate-200"}`}><p className="whitespace-pre-wrap break-words text-sm leading-5">{message.content ?? t("Message deleted")}</p><p className={`mt-1 text-end text-[10px] ${own ? "text-slate-700" : "text-slate-600"}`}>{message.delivery === "sending" ? t("Sending…") : readMessageId === message.id ? t("Read") : formatShortTime(message.createdAt)}</p>{message.delivery === "failed" ? <button className="mt-1 text-xs font-semibold text-rose-300 underline" onClick={() => onRetry(message)} type="button">{t("Retry")}</button> : null}</div></div>; })}</div>}</div>;
}

function Composer({ onChange, onSend, value }: Readonly<{ onChange: (value: string) => void; onSend: () => void; value: string }>) { const { t } = useLanguage(); return <footer className="shrink-0 border-t border-white/8 p-3 sm:p-4"><form className="flex items-center gap-2 rounded-2xl border border-white/8 bg-white/[0.035] p-2 px-4" onSubmit={(event) => { event.preventDefault(); onSend(); }}><input aria-label={t("Write a message…")} autoComplete="off" className="min-w-0 flex-1 bg-transparent text-sm text-slate-100 outline-none placeholder:text-slate-600" maxLength={4000} onChange={(event) => onChange(event.target.value)} placeholder={t("Write a message…")} value={value} /><button aria-label={t("Send message")} className="grid size-10 place-items-center rounded-xl bg-teal-300 text-slate-950 transition hover:bg-teal-200 disabled:cursor-not-allowed disabled:opacity-40" disabled={!value.trim()} type="submit"><SendIcon /></button></form></footer>; }
function EmptyConversation() { const { t } = useLanguage(); return <div className="grid flex-1 place-items-center px-6 text-center"><div><span className="mx-auto grid size-16 place-items-center rounded-3xl border border-teal-300/15 bg-teal-300/8 text-teal-300"><ChatIcon /></span><h2 className="mt-5 text-xl font-semibold">{t("Choose a conversation")}</h2><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">{t("Select a chat from the sidebar or search for someone new.")}</p></div></div>; }
...
function ConnectionNotice({ message, onRetry }: Readonly<{ message: string; onRetry: () => void }>) { const { t } = useLanguage(); return <div className="mx-4 mt-3 rounded-xl border border-amber-300/20 bg-amber-300/8 px-3 py-2.5 text-xs text-amber-100" role="alert"><p>{message}</p><button className="mt-2 font-bold text-amber-200 underline underline-offset-2" onClick={onRetry} type="button">{t("Retry connection")}</button></div>; }
