    ...
    <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/70 p-3 backdrop-blur-sm sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section aria-labelledby="profile-title" aria-modal="true" className="profile-panel max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/10 bg-[#0b1220] p-5 shadow-2xl shadow-black/50 sm:max-h-[calc(100dvh-3rem)] sm:p-7" role="dialog">
        <header className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-300">{t("Account")}</p><h2 className="mt-1 text-2xl font-semibold" id="profile-title">{t("Your profile")}</h2><p className="mt-1 text-sm text-slate-500">{t("Update how people see you in Online Chat.")}</p></div>
        ...
        <form className="mt-7 space-y-4" onSubmit={handleSave}>
          <div className="flex items-center gap-5 rounded-2xl border border-white/8 bg-white/[0.025] p-4">
            ...
            <div className="min-w-0 flex-1"><p className="text-sm font-semibold">{t("Profile photo")}</p><p className="mt-1 text-xs leading-5 text-slate-500">{t("Choose a PNG, JPG, WebP, or GIF image up to 2 MB.")}</p><div className="mt-3 flex flex-wrap gap-2"><label className="cursor-pointer rounded-xl bg-teal-300 px-3 py-2 text-xs font-bold text-slate-950 hover:bg-teal-200 focus-within:ring-2 focus-within:ring-teal-200"><input accept="image/jpeg,image/png,image/webp,image/gif" className="sr-only" onChange={(event) => { handleAvatarFile(event.target.files?.[0]); event.target.value = ""; }} type="file" />{t("Upload image")}</label>{avatarUrl ? <button className="rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-slate-400 hover:bg-white/5" onClick={() => { setAvatarUrl(""); setStatus({ kind: "success", message: "Image removed. Save your profile to apply this change." }); }} type="button">{t("Remove")}</button> : null}</div></div>
          </div>
        ...
        <section className="mt-8 rounded-2xl border border-rose-400/20 bg-rose-400/[0.055] p-4">
          <h3 className="font-semibold text-rose-200">{t("Delete account")}</h3><p className="mt-1 text-sm leading-5 text-slate-400">{t("Your identity will be anonymized. Message history remains for other conversation members. This cannot be undone.")}</p>
          <label className="mt-4 block"><span className="mb-2 block text-xs font-medium text-slate-400">{t("Type DELETE to confirm")}</span><input autoComplete="off" className="h-10 w-full rounded-xl border border-rose-400/20 bg-slate-950/30 px-3 text-sm outline-none focus:border-rose-400/60" onChange={(event) => setConfirmation(event.target.value)} value={confirmation} /></label>
          <button className="mt-3 h-10 w-full rounded-xl bg-rose-500 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-35" disabled={!isDeletionConfirmed(confirmation) || isDeleting || isSaving} onClick={handleDelete} type="button">{t(isDeleting ? "Deleting…" : "Permanently delete account")}</button>
        </section>
      </section>
    </div>
  );
